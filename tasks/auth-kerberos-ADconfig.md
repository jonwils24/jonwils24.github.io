...
In these instructions, it is important to identify each of the servers in your Alfresco cluster that will be running one or both of the Alfresco repository tier \(alfresco.war\) and Alfresco Share \(share.war\) web applications. Please see the [Configuring Share clustering](cluster-share-config.dita) topic for supported cluster configurations. These instructions should also apply to simple non-clustered installations, where a single alfresco.war and share.war run on a single host.
